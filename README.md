![Banner](https://github.com/SamuelSanchez17/SamuelSanchez17/blob/main/gojo.gif?raw=true)

<h1 align="center">Samuel Sanchez</h1>
<h3 align="center">Full-Stack Developer · Mexico 🇲🇽</h3>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=samuelsanchez17&label=Profile%20views&color=f5a623&style=flat" alt="samuelsanchez17" />
  <a href="https://samuelsanchez.vercel.app">
    <img src="https://img.shields.io/badge/Portfolio-samuelsanchez.vercel.app-f5a623?style=flat&logo=vercel&logoColor=white" />
  </a>
  <a href="https://www.linkedin.com/in/samuel-sanchez-guzman">
    <img src="https://img.shields.io/badge/LinkedIn-samuel--sanchez--guzman-0077B5?style=flat&logo=linkedin&logoColor=white" />
  </a>
</p>

---

## About

Full-Stack Developer with 2+ years of experience building production software for real clients — from desktop apps with CI/CD pipelines to REST APIs and mobile applications. I work across the full stack with Java, JavaScript, TypeScript, Swift and Rust, with a focus on clean architecture, maintainability, and real-world impact.

- 🖥️ Currently building [StockBeauty](https://github.com/SamuelSanchez17) — a desktop POS system in production with CI/CD via GitHub Actions + Tauri autoupdater
- 📱 Built MamLingua, an iOS app actively used by 300+ university students to preserve the Mam language
- 🔐 Developed a lab access control system processing 500+ monthly records with optimized PostgreSQL queries
- 📫 Reach me at **samuel.sanchezdev17@gmail.com**

---

## Tech Stack

### Languages
[![Skills](https://skillicons.dev/icons?i=java,js,ts,swift,python,rust,sql)](https://skillicons.dev)

### Frameworks & Libraries
[![Skills](https://skillicons.dev/icons?i=spring,react,vue,angular)](https://skillicons.dev)

### Databases
[![Skills](https://skillicons.dev/icons?i=mysql,postgres,sqlite)](https://skillicons.dev)

### Tools & Platforms
[![Skills](https://skillicons.dev/icons?i=git,github,tauri,postman,figma,vscode)](https://skillicons.dev)

---

## GitHub Stats

<p align="center">
  <img height="180em" src="https://github-readme-stats-eight-theta.vercel.app/api?username=SamuelSanchez17&show_icons=true&theme=algolia&include_all_commits=true&count_private=true"/>
  <img height="180em" src="https://github-readme-stats-eight-theta.vercel.app/api/top-langs/?username=SamuelSanchez17&layout=compact&langs_count=8&theme=algolia"/>
</p>

---

## Certifications

| Certificate | Issuer | Year |
|---|---|---|
| App Development with Swift Associate | Apple | 2024 |
| Swift Community Change Makers — Hackathon Nacional | Enactus México | 2024 |
| Google Cloud Computing Foundations | Google Cloud | 2024 |
| Semillero de Talento Java | HITSS México | 2025 |
| Desarrollo con IA: de 0 a producción | BIG School | 2026 |

---

<p align="center">
  <a href="https://samuelsanchez.vercel.app">🌐 Portfolio</a> ·
  <a href="https://www.linkedin.com/in/samuel-sanchez-guzman">💼 LinkedIn</a> ·
  <a href="mailto:samuel.sanchezdev17@gmail.com">✉️ Email</a>
</p>
